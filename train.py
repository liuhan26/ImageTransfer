...
def train_ac_gan():
    ...
    net_g, _ = generator(tf.concat([z_noise, net_z_classes.outputs], 1), is_train=True, reuse=False)
    ...
    net_g2, _ = generator(tf.concat([z_noise, net_z_classes.outputs], 1), is_train=False, reuse=True)
    ...
    g_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_fake, labels=tf.ones_like(d_logits_fake)))
    g_loss_class = tl.cost.cross_entropy(d_logits_fake_class, z_classes, name='ca')
